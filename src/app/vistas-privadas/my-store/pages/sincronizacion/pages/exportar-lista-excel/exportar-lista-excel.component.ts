...
export class ExportarListaExcelComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.path =
      URL_SERVICIOS +
      `/api/users/${this.userId}/stores/${this.storeId}/dowload_productcsv`
    console.log(this.path)

    this.sincronizacion
      .DowloadFileExcelListProduct(
  ...
  enviarExcel() {
    ...
    const file = new DocumentExcel(this.archivo)

    console.log('Archivo', file)

    this.sincronizacion
      .PostListadoProductosExcel(
        localStorage.getItem('id'),
        localStorage.getItem('storeId'),
        file
      )
