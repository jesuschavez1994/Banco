...
import {
  FavoriteResp,
  AddFavoriteProductResp,
  ProductsResponse,
  RemoveFavoriteProductResp,
...
export class ProductService extends Service {
  ...
  public getProductsByStore(
    idStore: any,
    page: number = 1,
    filter?: FilterProductResp
  ): Observable<ProductsResponse> {
    if (filter && Object.keys(filter).length > 0) {
      return this.postQuery<ProductsResponse>(
        `stores/${idStore}/products_search?page=${page}`,
        filter
      );
    }

    return this.execQuery<ProductsResponse>(
      `stores/${idStore}/products?page=${page}`
    );
  }

  public getProductByStore(
    idStore: number,
    idProduct: number
  ): Observable<Product> {
  ...
  public addProductToFavorite(
    idProduct: number
  ): Observable<AddFavoriteProductResp> {
    this.setIdUser();
    return this.postQuery<AddFavoriteProductResp>(
      `consumers/${this.idUser}/products/${idProduct}/favorites`,
      {}
    );
  }
  ...
  public removeProductFromFavorite(
    idProduct: number
  ): Observable<RemoveFavoriteProductResp> {
    this.setIdUser();
    return this.DeleteQuery<RemoveFavoriteProductResp>(
      `consumers/${this.idUser}/products/${idProduct}/favorites`
    );
  }
